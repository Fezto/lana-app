...
import { useCreateCategory, useListCategories, useUpdateCategory } from "@api/categories";
import { useUpdateBudget, useListBudgets } from "@api/budgets";
import {
  CategoryType,
  type CategoryRead,
  type BudgetRead,
...
export function EditBudgetForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      monthYear: budget.month_year || new Date().toISOString().slice(0, 7),
    },
  });
  ...
  // Encontrar la categoría actual del presupuesto
  ...
  const validateBudgetNotExists = (categoryId: number, monthYear: string) => {
    ...
    const budgetExists = existingBudgets.some(
      (existingBudget) =>
        existingBudget.category_id === categoryId && 
        existingBudget.month_year === monthYear &&
        existingBudget.id !== budget.id // Excluir el presupuesto actual
    );
  ...
  const onSubmit = async (data: EditBudgetFormData) => {
    try {
      ...
      // Validar que no exista el presupuesto (excluyendo el actual)
      ...
      setError("Error al actualizar el presupuesto. Inténtalo de nuevo.");
    }
  };
      ...
      <ScrollView style={styles.container}>
        ...
        <Card style={styles.currentCategoryCard}>
          <Card.Content>
            ...
            <Text variant="titleMedium">
              {currentCategory?.name || `Categoría ${budget.category_id}`}
            </Text>
        ...
        <SegmentedButtons
          ...
          onValueChange={(value) => {
            ...
            if (value === "existing") {
              setValue("selectedCategoryId", budget.category_id);
            } else {
